...
export default function PublicMenuPage() {
  ...
  useEffect(() => {
    ...
    document.documentElement.style.setProperty("--primary-foreground", getFg(primary))
    document.documentElement.style.setProperty("--secondary", secondary)
    document.documentElement.style.setProperty("--secondary-foreground", getFg(secondary))
  ...
  const updateQuantity = (id: string, qty: number, comment?: string) => update(id, qty, comment)

  const total = cart.reduce((t, i) => t + i.price * i.quantity, 0)

  const sendWhatsApp = () => {
    const items = cart
      .map((i) => `${i.quantity}x ${i.name}${i.comment ? ` (${i.comment})` : ''}`)
      .join("%0A")
    ...
    <div className="p-4 space-y-4">
      ...
      {mode === "delivery" && (
        <div ref={cartAnchorRef} className="space-y-4">
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  <Input
                    ...
                    value={item.quantity}
                    min={1}
                    onChange={(e) => updateQuantity(item.id, Number(e.target.value), item.comment)}
                  />
          ...
          <Input placeholder="Comentarios" value={orderComments} onChange={(e) => setOrderComments(e.target.value)} />
