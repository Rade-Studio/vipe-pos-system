  ...
  useEffect(() => {
    async function loadOrderData() {
      ...
      try {
        ...
        const tableInfo = await repositories.tables.getById(order.table_id)

        // Obtener información del mesero
        const waiterInfo = await repositories.profiles.getById(order.waiter_id)
  ...
  const processPayment = async () => {
    ...
    try {
      ...
      if (transaction) {
        ...
        try {
          invoiceNumber = await repositories.orders.completePayment(orderId, paymentMethod)
          if (isPartialPayment) {
