# Delta for Payment and Order Atomicity (P3)
...
#### Scenario: S3-01 — Happy path: payment completes and frees table
...
#### Scenario: S3-02 — Double-tap produces exactly one payment row
...
#### Scenario: S3-05 — Junction table links orders to ingredient consumption
...
#### Scenario: S3-06 — Multi-method payment atomicity
...
None — P3 introduces new behavior only.
...
Rollback: `DROP FUNCTION complete_payment(uuid, text, numeric);` and `DROP FUNCTION delete_order_with_items(uuid);` — reversible; client retains old `service.ts` code path during feature-flag window.
