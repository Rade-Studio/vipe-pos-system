# Delta for Multi-Tenant RLS and Auth (P2) — investigar-mesa-sync-realtime
...
> **Apply note**: All P2 migrations are applied via `supabase db reset` (user-confirmed). No idempotent shim migration is used. The fresh-volume init is handled by `supabase/init.sql` (IF-02).
...
#### Scenario: R2-01-S1 — Tenant FK present on all tenant tables
...
#### Scenario: R2-01-S2 — Tenant isolation: anon key A cannot read tenant B rows
...
#### Scenario: R2-02-S1 — RLS SELECT policy enforces tenant scope
...
#### Scenario: R2-02-S2 — RLS INSERT policy blocks cross-tenant write
...
#### Scenario: R2-03-S1 — Default anon grant removed
...
#### Scenario: R2-04-S1 — New auth user gets auto-created profile
...
#### Scenario: R2-04-S2 — Role PIN no longer gates role views
...
#### Scenario: R2-05-S1 — Password column removed
...
#### Scenario: R2-06-S1 — Profile links to auth user
...
#### Scenario: R2-07-S1 — Anon cannot upload dish image
...
**Apply**: `supabase db reset` (user-confirmed; destroys existing DB data; all 7 migrations apply in order)
...
**Rollback**: Reverting P2 requires reverting migration order in a single down script (drop policies → drop FKs → restore columns nullable → restore `init.sql` defaults). Not applied incrementally.
