-- Crear un bucket público para almacenar imágenes
CREATE POLICY "Public Access"
ON storage.objects FOR SELECT
USING (bucket_id = 'images' OR bucket_id = 'dishes');

-- Permitir a usuarios autenticados insertar en el bucket
CREATE POLICY "Authenticated Users Can Upload"
ON storage.objects FOR INSERT
WITH CHECK (bucket_id IN ('images', 'dishes') AND auth.role() = 'authenticated');
...
USING (bucket_id IN ('images', 'dishes') AND auth.uid() = owner);
